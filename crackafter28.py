...
#    En.append()
    ...
    fddd=McrN/550
    IcrN=B*(layer*i)**3/12+B*(layer*i)*(layer*i/2)**2+B/ne*6**3/12+B/ne*6*(H0-layer*i)**2+24/ne*74**3/12+74/ne*24*(H1-layer*i-74/2)**2
    IeN=IcrN+(McrN/M01)**3*(Ig-IcrN)
    nd=M01*(3*l*l-4*a*a)/24/Ec[1]/IeN
    return En,i,strain

#参数部分    
fc=29
fr=2.80
N=[0,0.0001,1.0,3.0,10.0,50.0,100.0,150.0,200.0]
#'N=list(N)'
...
M01=M00                                 #'M01为加载上限弯矩
F=[50000,50000,20000,30000,40000,50000,60000,70000,80000,90000,100000,200000,300000]
"""
print 'hello'
...
for tt in range(1,9):
    ...
    Ec,i,test1=main(fc,fr,N,t,layer,B,H,Eg,Ig,Ec,tt,M00,M01)
    Ec.append(33000)
    Ec.append(33000)
    Ec.append(33000)
    Ec.append(33000)
    ...
    IcrN=B*(layer*i)**3/12+B*(layer*i)*(layer*i/2)**2+B*6**3/12+B*6*(H-layer*i)**2           # '前三个2为分层厚度,
    McrN=Ig*frN/(H-layer*i)    
    IeN=IcrN+(McrN/M01)**3*(Ig-IcrN)
